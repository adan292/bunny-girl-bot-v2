...
// DETECTOR ANTILINK (CONECTADO A LA BASE DE DATOS)
async function checkAntiLink(sock, msg) {
  try {
    ...
    const textContent = text(msg);
    const linkRegex = /(https?:\/\/[^\s]+|www\.[^\s]+|chat\.whatsapp\.com\/[^\s]+)/i;
    if (!linkRegex.test(textContent)) return;

    const sender = msg.key.participant || msg.participant;
    if (!sender) return;

    // Verificar si el remitente es admin o el propio bot
    if (sender === sock.user.id || await isAdmin(sock, from, sender)) return;

    // El bot necesita ser administrador para eliminar mensajes
    if (!await botAdmin(sock, from)) return;
...
module.exports = { execute, text, mentioned, target, isUrl, checkAntiLink };
